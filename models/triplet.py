...
class TripletLossFilter(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        for i in range(len(targets)):
            if targets[i] == -1 or targets[i]==20000 or torch.sum(inputs[i])==0:
                continue
            # elif targets[i]==20000:
            #    inputs_new.append(inputs[i])
            #    targets_new.append(targets[i]+no_id_idx)
            #    targets_value.append(targets[i].cpu().numpy().item()+no_id_idx)
            #    no_id_idx+=1
            else:
                inputs_new.append(inputs[i])
