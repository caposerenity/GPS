...
class SeqNet(nn.Module):
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        if targets is not None and not query_img_as_gallery:
            ...
            box_features = self.roi_heads.reid_head(box_features, 0, False)
    ...
    def forward(self, images, targets=None, images_sur=None, targets_sur=None, query_img_as_gallery=False):
        if not self.training:
            return self.inference(images, targets, query_img_as_gallery)

        images, targets = self.transform(images, targets)
        #images_sur, targets_sur = self.transform(images_sur, targets_sur )

        features = self.backbone(images.tensors)
        proposals, proposal_losses = self.rpn(images, features, targets)
        _, detector_losses = self.roi_heads(features, proposals, images.image_sizes, targets)

        # features_sur = self.backbone(images_sur.tensors)
        # proposals_sur, proposal_losses_sur = self.rpn(images_sur, features_sur, targets_sur)
        # _, detector_losses_sur = self.roi_heads(features_sur, proposals_sur, images_sur.image_sizes, targets_sur)
        ...
        # proposal_losses_sur["loss_rpn_reg_sur"] = proposal_losses_sur.pop("loss_rpn_box_reg")
        # proposal_losses_sur["loss_rpn_cls_sur"] = proposal_losses_sur.pop("loss_objectness")
        # detector_losses_sur["loss_box_reg_sur"] = detector_losses_sur.pop("loss_box_reg")
        # detector_losses_sur["loss_box_cls_sur"] = detector_losses_sur.pop("loss_box_cls")
        # detector_losses_sur["loss_proposal_reg_sur"] = detector_losses_sur.pop("loss_proposal_reg")
        # detector_losses_sur["loss_proposal_cls_sur"] = detector_losses_sur.pop("loss_proposal_cls")
        # detector_losses_sur["loss_box_reid_sur"] = detector_losses_sur.pop("loss_box_reid")
        
        # for k in detector_losses_sur.keys():
        #     detector_losses_sur[k]*=0.0
        # for k in proposal_losses_sur.keys():
        #     proposal_losses_sur[k]*=0.0
        for k in detector_losses.keys():
            detector_losses[k]*=1.0
        for k in proposal_losses.keys():
            proposal_losses[k]*=1.0

        losses = {}
        # losses.update(detector_losses_sur)
        # losses.update(proposal_losses_sur)
        ...
        losses["loss_cov"] *= 100
        losses["loss_triplet"] *= 1.0

        '''losses["loss_rpn_reg_sur"] *= self.lw_rpn_reg
        losses["loss_rpn_cls_sur"] *= self.lw_rpn_cls
        losses["loss_proposal_reg_sur"] *= self.lw_proposal_reg
        losses["loss_proposal_cls_sur"] *= self.lw_proposal_cls
        losses["loss_box_reg_sur"] *= self.lw_box_reg
        losses["loss_box_cls_sur"] *= self.lw_box_cls
        losses["loss_box_reid_sur"] *= self.lw_box_reid'''
        return losses
...
class SeqRoIHeads(RoIHeads):
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        if self.training:
        #if False:
            proposal_features = self.box_head(proposal_features, targets[0]["flag"], self.training)
        else:
            proposal_features = self.box_head(proposal_features, 0, self.training)
        ...
        if not self.training and query_img_as_gallery:
            ...
            gt_box_features = self.reid_head(gt_box_features, 0, False)
        ...
        if self.training:
        #if False:
            box_features = self.reid_head(box_features, targets[0]["flag"], self.training)
        else:
            box_features = self.reid_head(box_features, 0, self.training)
        ...
        if self.training:
            ...
            for i in range(len(targets)):
                name = os.path.basename(targets[i]['img_name'])
                frame_idxes.append(int(name[:-4]))
            #     video_ids = torch.cat((video_ids, torch.zeros(len(box_pid_labels[i]))+targets[i]['video_id']))
            loss_box_reid, loss_cov, loss_triplet = self.reid_loss(box_embeddings, box_pid_labels, frame_idxes)
            losses.update(loss_box_reid=loss_box_reid, loss_cov=loss_cov, loss_triplet=loss_triplet)
        else:
