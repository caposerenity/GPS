...
class CUHKSYSU(BaseDataset):
    def __init__(self, root, transforms, split, append):
        self.name = "CUHK-SYSU"
        self.img_prefix = osp.join(root, "Image", "SSM")
        self.is_append_dataset = append
    ...
    def _load_annotations(self):
        ...
        name_to_boxes = {}
        name_to_pids = {}
        unlabeled_pid = 20001  # default pid for unlabeled people
        ...
        for img_name in imgs:
            ...
            if self.is_append_dataset:
                pids[pids != 20001]+=10141
            annotations.append(
                {
                    "img_name": img_name,
                    "img_path": osp.join(self.img_prefix, img_name),
        ...
        self._set_group_flag(len(annotations))
        return annotations
    
    def _set_group_flag(self, l):
        """Set flag according to image aspect ratio.

        Images with aspect ratio greater than 1 will be set as group 1,
        otherwise group 0.
        """
        self.flag = np.zeros(l, dtype=np.uint8)+3
    ...
    def __getitem__(self, index):
        ...
        target = {"img_name": anno["img_name"], "boxes": boxes, "labels": labels, "idx":index,
                   "video_id": 5555, 
                   "flag": torch.tensor([3], dtype=torch.int64)}
