...
from .prw import PRW
from .lup_dg import LUPDG
from .group_sampler import DistributedGroupSampler, get_dist_info
...
def build_dataset(dataset_name, root, transforms, split, append=False, verbose=True):
    if dataset_name == "CUHK-SYSU":
        dataset = CUHKSYSU(root, transforms, split, append)
    elif dataset_name == "PRW":
        dataset = PRW(root, transforms, split, append)
    elif dataset_name == "LUP-DG":
        dataset = LUPDG(root, transforms)
    else:
        raise NotImplementedError(f"Unknow dataset: {dataset_name}")
    if verbose and dataset_name != "LUP-DG":
...
def build_train_loader(cfg, dataset=None, sur=False, epoch=0):
    if dataset==None:
    ...
    sampler = None
    if cfg.INPUT.DATASET == "LUP-DG" and not sur:
        sampler = DistributedGroupSampler(
                    dataset, cfg.INPUT.BATCH_SIZE_TRAIN, world_size, rank, seed=cfg.SEED)
        #sampler.set_epoch(epoch)
        #print('set epoch: '+str(epoch))
    return torch.utils.data.DataLoader(
        dataset,
        batch_size=cfg.INPUT.BATCH_SIZE_TRAIN,
        sampler=sampler,
        shuffle= sur, #(cfg.INPUT.DATASET != "LUP-DG"),
...
def build_test_loader(cfg):
    ...
    if cfg.INPUT.DATASET == "LUP-DG":
        #cfg.INPUT.DATASET = 'CUHK-SYSU'
        #cfg.INPUT.DATA_ROOT = '/dockerdata/path/data/CUHK-SYSU/'
        gallery_set = build_dataset('CUHK-SYSU', '/dockerdata/path/data/CUHK-SYSU/',transforms, "gallery")
        query_set = build_dataset('CUHK-SYSU', '/dockerdata/path/data/CUHK-SYSU/', transforms, "query")
    else:
