...
def train_one_epoch(cfg, model, optimizer, data_loader, train_loader_sur, device, epoch, tfboard=None, scaler=None):
    model.train()
    ...
    # for i, ((images, targets), (images_sur, targets_sur)) in enumerate(
    #     metric_logger.log_every(zip(data_loader, train_loader_sur), cfg.DISP_PERIOD, header)
    # ):
    for i, (images, targets) in enumerate(
        metric_logger.log_every(data_loader, cfg.DISP_PERIOD, header)
    ):
        images, targets = to_device(images, targets, device)
        #images_sur, targets_sur = to_device(images_sur, targets_sur, device)

        loss_dict = model(images, targets)
        #loss_dict = model(images, targets, images_sur, targets_sur)
        ...
        optimizer.step()
        dist.all_reduce(model.module.roi_heads.reid_loss.lut, op=dist.ReduceOp.SUM)
        model.module.roi_heads.reid_loss.lut /= dist.get_world_size()
...
def evaluate_performance(
    ...
    if use_cache:
        ...
        # [:,model.roi_heads.reid_loss.id_relevant_dims]
